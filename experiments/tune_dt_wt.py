# tune_dt_wp_sgkfold_all_setups.py
# ---------------------------------------------------------
# Decision Tree + Optuna com StratifiedGroupKFold (4 folds)
...
def make_objective(all_train_data, n_splits=4, seed=42):
    """
    Creates the Optuna goal::
      - For each trial, evaluate DT with StratifiedGroupKFold in each setup
      - Returns the average CV accuracy between setups
    """
...
def main():
    ...
    # Optuna: objective -> average CV accuracy across the 10 setups
    objective = make_objective(all_train_data, n_splits=4, seed=42)
    study = optuna.create_study(direction="maximize", study_name="dt_wp_sgkfold_all_setups")
    ...
    for pack_train, pack_test in zip(all_train_data, all_test_data):
        ...
        clf = DecisionTreeClassifier(**{**bp, "random_state": 42})
        clf.fit(X_tr, y_tr)
    ...
    # Save best params
    with open("best_params/dt_best_params.json", "w") as f:
